...
INSERT INTO
    tbl_role_permissao (id_role, id_permissao)
SELECT
    (
        SELECT
            id_role
...
WHERE
    NOT (
        ...
        AND acao IN ('criar', 'editar', 'excluir')
    );
...
WHERE
    ...
    OR (
        recurso = 'clientes'
        AND acao IN ('visualizar', 'criar', 'editar')
    )
    OR (
        recurso IN ('entradas', 'saidas')
        AND acao IN ('visualizar', 'criar')
    )
    OR (
        recurso IN ('ajustes', 'devolucoes')
        AND acao IN ('visualizar', 'criar')
    )
    OR (
        recurso = 'dashboard'
...
VALUES
    (
        ...
        'admin',
        '$2b$10$N.9VpRKHQ8QaUQjTcp8ROuWGEM1n.XMfYSsQMfg5aHuDQCDb.HX9C',
        (
            SELECT
                id_role
            FROM
...
CREATE TABLE
    tbl_lote (
        ...
        FOREIGN KEY (id_item_entrada) REFERENCES tbl_item_entrada (id_item_entrada),
...
CREATE TABLE
    tbl_saida_lote (
        id_saida_lote INT AUTO_INCREMENT PRIMARY KEY,
        id_item_saida INT NOT NULL,
        id_lote INT NOT NULL,
        quantidade INT NOT NULL,
        FOREIGN KEY (id_item_saida) REFERENCES tbl_item_saida (id_item_saida),
        FOREIGN KEY (id_lote) REFERENCES tbl_lote (id_lote),
        CHECK (quantidade > 0)
    );

CREATE INDEX idx_lote_produto_disponivel ON tbl_lote (id_produto, quantidade_atual);

CREATE INDEX idx_lote_validade ON tbl_lote (data_validade);